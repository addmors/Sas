...
namespace Sas {
	BuilderNode::BuilderNode(ImTextureID texture, int textureWidth, int textureHeight)
		: HeaderTextureId(texture),
		HeaderTextureWidth(textureWidth),
		HeaderTextureHeight(textureHeight) {}

	void BuilderNode::Begin(ax::NodeEditor::NodeId id)
	{
		HasHeader = false;
	...
	void BuilderNode::Header(const ImVec4& color)
	{
		HeaderColor = ImColor(color);
		SetStage(Stage::Header);
	}

	void BuilderNode::EndHeader()
	{
		SetStage(Stage::Content);
	}

	void BuilderNode::Input(ax::NodeEditor::PinId id)
	{
	...
	void BuilderNode::Output(ax::NodeEditor::PinId id)
	{
		if (CurrentStage == Stage::Begin)
			SetStage(Stage::Content);
	...
	bool BuilderNode::SetStage(Stage stage)
	{
	...
	void BuilderNode::Pin(ax::NodeEditor::PinId id, ax::NodeEditor::PinKind kind)
	{
		ed::BeginPin(id, kind);
	}

	void BuilderNode::EndPin()
	{
		ed::EndPin();
	}
