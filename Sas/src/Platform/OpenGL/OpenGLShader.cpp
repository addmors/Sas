...
namespace Sas {
	...
	void OpenGLShader::Reflect(GLenum stage, const std::vector<uint32_t>& shaderData)
	{
		...
		SS_CORE_TRACE("OpenGLShader::Reflect - {0} {1}", Utils::GLShaderStageToString(stage), m_FilePath);
		SS_CORE_TRACE("    {0} uniform buffers", resources.uniform_buffers.size());
		SS_CORE_TRACE("    {0} resources", resources.sampled_images.size());

		SS_CORE_TRACE("Uniform buffers:");
		for (const auto& resource : resources.uniform_buffers)
		{
			...
			SS_CORE_TRACE("  {0}", resource.name);
			SS_CORE_TRACE("    Size = {0}", bufferSize);
			SS_CORE_TRACE("    Binding = {0}", binding);
			SS_CORE_TRACE("    Members = {0}", memberCount);
		}
	}
